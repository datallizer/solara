...
if (isset($_SESSION['codigo'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $queryubicacion = "UPDATE `usuarios` SET `ubicacion` = 'Editando encargado de proyecto' WHERE `usuarios`.`codigo` = '$codigo'";
...
<head>
    ...
    <title>Encargados de Proyecto | Solara</title>
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <h4>ENCARGADOS DE PROYECTO <button type="button" class="btn btn-primary btn-sm float-end m-1" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                        Asignar encargado
                                    </button></h4>
                            </div>
                            <div class="card-body" style="overflow-y:scroll;">
                                ...
                                <table id="miTabla" class="table table-bordered table-striped" style="width: 100%;">
                                    ...
                                    <tbody>
                                        <?php
                                        $query = "SELECT encargadoproyecto.*, proyecto.id, encargadoproyecto.id AS id_encargado, proyecto.nombre,usuarios.nombre AS username, usuarios.apellidop, usuarios.apellidom
                                        FROM encargadoproyecto
                                        JOIN usuarios ON encargadoproyecto.codigooperador = usuarios.codigo
                                        JOIN proyecto ON encargadoproyecto.idproyecto = proyecto.id 
                                        WHERE encargadoproyecto.codigooperador = usuarios.codigo
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <form action="codencargados.php" method="POST" class="d-inline">
                                                            <button type="submit" name="deleteproyecto" value="<?= $registro['id_encargado']; ?>" class="btn btn-danger btn-sm m-1"><i class="bi bi-trash-fill"></i></button>
                                                        </form>
                                                    </td>
                                                </tr>
                                        <?php
